          ...
          <div class="modal-body">
            <form @submit.prevent="addUser">
              ...
              <div class="form-group">
                ...
                <input v-model="newUser.username" type="text" class="form-control" id="username" required>
              ...
              <button type="button" class="btn btn-secondary" @click="showAddModal = false">Đóng</button>
            </form>
          </div>
        </div>
      </div>
          ...
          <div class="modal-body">
            <form @submit.prevent="updateUser">
              <div class="form-group">
                ...
                <input v-model="editUser.name" type="text" class="form-control" id="editName" required>
                <span class="text-danger">{{ errorEditName }}</span>
              </div>
              <div class="form-group">
                <label for="editUsername">Tên đăng nhập</label>
                <input v-model="editUser.username" type="text" class="form-control" id="editUsername" required>
                <span class="text-danger">{{ errorEditUsername }}</span>
              </div>
              <div class="form-group">
                <label for="editEmail">Email</label>
                <input v-model="editUser.email" type="email" class="form-control" id="editEmail" required>
                <span class="text-danger">{{ errorEditEmail }}</span>
              </div>
              <div class="form-group">
                <label for="editRole">Vai trò</label>
                <select v-model="editUser.role" class="form-control" id="editRole" required>
                  <option value="1">Quản trị viên</option>
                  <option value="2">Người dùng</option>
                  <option value="3">Khách</option>
                </select>
 ...
 <div class="modal fade" :class="{ show: showDeleteModal }" tabindex="-1" role="dialog" style="display: block;" v-if="showDeleteModal">
  ...
  <tbody>
    <tr v-for="user in users" :key="user.id">
      ...
      <td>{{ user.role }}</td>
      <td>
        <button class="btn btn-warning" @click="editUserModal(user)">Sửa</button>
        <button class="btn btn-danger" @click="confirmDeleteModal(user)">Xóa</button>
      </td>
    </tr>
  </tbody>
</table>
...
export default {
  ...
  methods: {
    ...
    validateField(fieldNameError, fieldName, fieldValue, minLength, maxLength, regexPattern, existingValues) {
  ...
  if (fieldNameError === "Password") {
    if (fieldValue.length < minLength || fieldValue.length > maxLength) {
      this[`error${fieldNameError}`] = `${fieldName} phải có từ ${minLength} đến ${maxLength} ký tự`;
      return false;
    }
    if (/\s/.test(fieldValue)) {
      this[`error${fieldNameError}`] = `${fieldName} không được chứa khoảng trắng`;
      return false;
    }
  } 
  ...
  if (fieldNameError === "Username") {
    ...
    if (!hasLetter || !hasNumber) {
      this[`error${fieldNameError}`] = `${fieldName} phải chứa ít nhất một chữ và một số`;
      return false;
    }
  ...
  const isValidName = this.validateField("Name", "Tên", this.newUser.name, 5, 100, regex);
  const isValidUsername = this.validateField("Username", "Tên đăng nhập", this.newUser.username, 5, 20, regex, existingUsernames);
  const isValidPassword = this.validateField("Password", "Mật khẩu", this.newUser.password, 8, 30, null);
  const isValidEmail = this.validateField("Email", "Email", this.newUser.email, 5, 100, null, existingEmails);
  ...
  if (isValidName && isValidUsername && isValidEmail && isValidPassword) {
    try {
      ...
      if (error.response && error.response.status === 422) {
        ...
        this.errorUsername = validationErrors.username ? validationErrors.username[0] : '';
        this.errorEmail = validationErrors.email ? validationErrors.email[0] : '';
      } else {
    ...
    async updateUser() {
  const regex = /^(?!.*\s{2})(?!.*\u3000)[\p{L}0-9\s]+$/u;
  const isValidName = this.validateField("EditName", "Tên", this.editUser.name, 5, 100, regex);
  const isValidUsername = this.validateField("EditUsername", "Tên đăng nhập", this.editUser.username, 5, 20, regex);
  const isValidEmail = this.validateField("EditEmail", "Email", this.editUser.email, 5, 100, null);

  if (isValidName && isValidUsername && isValidEmail) {
    try {
      const response = await axios.put(`http://127.0.0.1:8000/api/users/${this.editUser.id}`, this.editUser); // Đảm bảo dùng PUT
      ...
      this.editUser = { id: '', name: '', username: '', email: '', role: '' };
      this.errorEditName = '';
    ...
    async deleteUser() {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/users/${this.userToDelete.id}`);
